...
echo $CLEAN_QRTZ
echo "----"
CLEAN_QRTZ2=$(mysql --host=$BDD_HOST --port=$BDD_PORT -u $BDD_USER --password=$BDD_PASSWORD -D $DB_SYNCHRO -e "DELETE FROM QRTZ_PARALLEL_JOB_DETAILS WHERE JOB_NAME NOT IN (SELECT JOB_NAME FROM QRTZ_PARALLEL_TRIGGERS) AND SCHED_NAME = 'parallelScheduler'")
echo $CLEAN_QRTZ2
