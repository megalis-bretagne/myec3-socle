...
public class UnstackUpdateList {
	...
	public void unstackUpdateResource() {
		...
		if (CronUtils.executeCronOnHost()) {
			...
			for (MpsUpdateJob resourceToUnstack : listToUnstack) {
				...
				if (hostName != null && hostName.equals(resourceToUnstack.getMachineName())) {
					...
					logger.info("Host has no right to execute requests : " + hostName);
					continue;
				}
			}
			...
			logger.info("Job failed : executeCronOnHost returned FALSE");
		}
	}
